...
abstract sealed class Tree[+A <% Ordered[A]] {
  ...
  def isValid: Boolean = {
    var pre: Tree[A] = Nil
    def inorderCheck(current: Tree[A]): boolean = {
      if (current == Nil) true

      if (!inorderCheck(current.left)) return false

      if (pre != Nil && pre.value > current.value) return false
      pre = current

      if (!inorderCheck(current.right)) return false
